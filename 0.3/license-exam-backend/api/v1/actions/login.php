...
//echo 'email and password after post ' . $email . '  pw:  ' . $password . "\n";
...
if (!isset($_SESSION['salt'])) {
    die('{"success": false, "error": "eroor fret"}');
}
...
$salt = $_SESSION['salt'];
//echo 'salt from session: ' . $salt . "\n" . 'password from table: ' . $password_from_table . "\n";
...
// Output the password from the frontend
//echo 'password from frontend: ' . $password . "\n";
...
if ($backed_hashed_password != $password) {
    ...
    die('{"success": false, "error": "Invalid email or password at hashing TODO"}');
}
